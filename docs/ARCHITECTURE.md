...
Modules: `data.py` parses and describes; `model.py` defines assumptions and simulation; `analysis.py` enumerates/evaluates; `friction.py` keeps user-entered warehouse constraints separate, translates only timed effects to explicit expected service-time additions and returns qualitative safety prompts; `report.py` creates reproducible bundles; `web.py` serves; `__main__.py` provides CLI. The UI uses the same API/core as CLI. All calculations are in tested Python.

The friction comparison is a second complete evaluation with adjusted service means. It does not change the base scenario or silently infer a delay from the uploaded CSV. Qualitative safety flags cannot affect capacity, payroll, risk or feasibility; they are explicit prompts to involve the site's responsible safety process.
